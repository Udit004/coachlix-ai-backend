...
import { getRecentChatHistory } from "../memory/chatMemory.js";
import { buildMinimalContext } from "./contextRetrieval.js";
...
function getRole(msg) {
  ...
  if (typeof msg._getType === 'function') {
    ...
    return type === 'human' ? 'user' : (type === 'ai' ? 'ai' : type);
  }
  if (msg.id && Array.isArray(msg.id)) {
    if (msg.id.includes('HumanMessage')) return 'user';
    if (msg.id.includes('AIMessage')) return 'ai';
  }
  return 'unknown';
}
...
function getContent(msg) {
  ...
  if (msg.kwargs && typeof msg.kwargs.content === 'string') return msg.kwargs.content;
  return '';
}
...
function rankMessagesByRelevance(messages, currentMessage, maxMessages = 10) {
  ...
  const scoredMessages = messages.map((message, index) => {
    let score = 0;
    const messageContent = getContent(message).toLowerCase();
    ...
    if (relevantMessages && relevantMessages.length > 0) {
      ...
      for (let i = 0; i < relevantMessages.length; i++) {
        ...
        if (item && item.message) {
          const role = getRole(item.message) === 'user' ? 'User' : 'Coachlix';
          const txt = getContent(item.message); const content = txt.substring(0, 150) + (txt.length > 150 ? '...' : '');
          recentExchanges.push(`${role}: ${content}`);
        }
      }
      
      recentExchanges.slice(0, 5).forEach((exchange, idx) => {
        enhancedContext += `\nPrevious Msg ${idx + 1}: ${exchange}\n`;
      });
    }
    
    if (conversationAnalysis && conversationAnalysis.topicsCovered && conversationAnalysis.topicsCovered.length > 0) {
