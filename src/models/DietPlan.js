...
DietPlanSchema.pre('save', function recalculateTotals(next) {
...
DietPlanSchema.methods.getAverageCalories = function getAverageCalories() {
...
DietPlanSchema.methods.getTotalMeals = function getTotalMeals() {
  return this.days.reduce((total, day) => total + day.meals.length, 0);
};

DietPlanSchema.methods.getDayByNumber = function getDayByNumber(dayNumber) {
  return this.days.find((day) => day.dayNumber === dayNumber);
};

DietPlanSchema.statics.findByUserId = function findByUserId(userId, options = {}) {
  const query = { userId };
...
DietPlanSchema.statics.findByGoal = function findByGoal(goal) {
  return this.find({ goal, isActive: true });
};

DietPlanSchema.statics.getActivePlan = function getActivePlan(userId) {
  return this.findOne({ userId, isActive: true });
};

DietPlanSchema.statics.setActivePlan = async function setActivePlan(userId, planId) {
  await this.updateMany(
    { userId, _id: { $ne: planId } },
    { $set: { isActive: false } }
  );

  return this.findByIdAndUpdate(planId, { $set: { isActive: true } }, { new: true });
};
